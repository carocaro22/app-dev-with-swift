...
func mealLog (userInput: String) -> Meal? {
    if let myMeal = meals[userInput] {
        return myMeal
    }
    else {
        return nil
...
if let myMeal = mealLog(userInput: "Breakfast") {
    print(myMeal)
}
if let myMeal = mealLog(userInput: "Dinner"){
    print(myMeal)
}
/*:
...
userDefaults.set(dictionary, forKey: "mealLog")

func isSaved () -> [String : Any]? {
    if let myMeal = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return myMeal
    }
    else {
        var newMealLog: [String : Any] = [:]
        return newMealLog
    }
}
if let savedMealLog = isSaved() {
    print(savedMealLog)
}
