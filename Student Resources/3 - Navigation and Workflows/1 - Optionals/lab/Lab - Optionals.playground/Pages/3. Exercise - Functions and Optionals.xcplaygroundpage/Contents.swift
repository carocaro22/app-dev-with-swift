...
func checkAge (age: String?) -> Int? {
    let possibleAge = Int(userInputAge)
    if let unwrappedAge = possibleAge {
        print(unwrappedAge)
        return possibleAge
    }
    else {
...
func checkPrice (userInput: String) -> Double? {
    if let inStock = stock[userInput] {
        if inStock > 0 {
            return prices[userInput]
        }
        else{
            print("not in stock")
            return nil
...
if let price = checkPrice(userInput: "Broccoli") {
    print(price)
}
